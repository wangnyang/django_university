...
class CreatedMixin(models.Model):
    created = models.DateTimeField(_('created'), default=get_datetime, blank=True, null=False, editable=False)

    class Meta:
        abstract = True


def validate_modified(modified: datetime):
    if modified > now():
        raise ValidationError(
            'The \'modified\' field cannot be date in the future',
            params={'modified': modified}
        )


class ModifiedMixin(models.Model):
    modified = models.DateTimeField(_('modified'), default=get_datetime, validators=[validate_modified], blank=True, null=False)
...
# TODO
# нужен ли валидатор, в базе же может быть и оценка 7
# но он не сможет ее проверить, потому что она давно добавлена
